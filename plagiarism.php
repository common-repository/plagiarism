...
public function render_meta_box(){
	...
	if (count($chunks)>0) {
	  ...
	  foreach($chunks as $key => $value){
	    ...
	    $url = $this->build_query($key, $plagiarism_search);
	    $link = '<a href="'.$url.'" target="plagiarism" title="'. $key.'" >'.$key.'</a>';
...
public function check_post($post_id) {
	...
	if ($check_ok){update_post_meta($post_id, 'plagiarism_duplicate', 'false');}
	else {update_post_meta($post_id, 'plagiarism_duplicate', 'true');}
...
public function get_google_count( $dom ){
	...
	if($docs_found){
    // CHECK FOR THE "NO RESULTS FOUND" STRING TO AVOID MISSPELLING
	  $xpath = new DOMXPath($dom);
	  $node = $xpath->query('//span[@class="spell_orig"]');
	  $text2 = $node->item(0)->textContent;
	  unset($xpath);
    if(substr($text2,0,10) == 'No results'){
      return 0;
    }
    // CHECK FOR THE "NO RESULTS FOUND" STRING TO AVOID WITHOUT QUOTES
    $no_result_without_quotes_needle = '<img src="/images/yellow-alert2x.png" alt="Information" style="margin-right:8px"height=14 width=14>No results found for ';
    if ($this->current_search_result != '' && strpos($this->current_search_result, $no_result_without_quotes_needle) !== false ) {
      return 0;
    }    
...
public function get_bing_count( $dom ){
	...
	$txt = $node->item(0)->textContent;
	if(stripos($txt,'no results found') !== false){
	  $text='';
	}
	unset($xpath);
...
public function search ($query) {
	...
	if (get_user_option('plagiarism_use_proxy') == 'on') {
	  ...
	  if (count($proxies) > 0) {
	    ...
	    print_r($proxies[$current_proxy]); echo '<br/>';
...
?>
		  ...
		  <form name="plagiarism_admin_form" id="plagiarism_admin_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
			...
			?>
		    <p class="submit"><input type="submit" class="button-primary" name="Submit" value="<?php _e('Update Options', LANG ) ?>" /></p>
		  </form>
	  </div>
